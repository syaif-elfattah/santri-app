...
export default function DashboardPage() {
  // SWR: cache otomatis, revalidate saat tab difokus kembali
  const { data: rekap,  isLoading: loadRekap  } = useSWR<Rekap[]>('/api/prestasi', fetcher, { refreshInterval: 30_000 })
  const { data: kelas,  isLoading: loadKelas  } = useSWR<Kelas[]>('/api/kelas', fetcher)
  const { data: taList                         } = useSWR('/api/tahun-ajaran', fetcher)

  const [filter, setFilter] = [
    (typeof window !== 'undefined' ? (new URLSearchParams(window.location.search).get('kelas') || '') : ''),
    (v: string) => {
      const url = new URL(window.location.href)
      v ? url.searchParams.set('kelas', v) : url.searchParams.delete('kelas')
      window.history.replaceState({}, '', url)
    }
  ]

  const [filterState, setFilterState] = require('react').useState('')

  const data     = rekap || []
  const filtered = filterState ? data.filter(d => d.kelas === filterState) : data
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            ...
            {taAktif && <span className="ml-2 text-emerald-600 font-medium">· TA {taAktif.nama}</span>}
          </p>
        </div>
        ...
        {[
          ...
          <div key={s.label} className="card">
            ...
            <div className="text-2xl font-semibold text-gray-900">
              {loadRekap ? <span className="animate-pulse bg-gray-200 rounded h-7 w-12 inline-block"/> : s.value}
            </div>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <button onClick={() => setFilterState('')}
          className={`tag-pill ${!filterState ? 'active' : ''}`}>Semua kelas</button>
        {kelasList.map(k => (
          <button key={k.id} onClick={() => setFilterState(k.nama)}
            className={`tag-pill ${filterState === k.nama ? 'active' : ''}`}>{k.nama}</button>
        ))}
      </div>
              ...
              {filtered.map((d, i) => (
                <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-400">
                    {d.terakhir_update
                      ? new Date(d.terakhir_update).toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'})
                      : '—'}
                  </td>
                </tr>
              ))}
