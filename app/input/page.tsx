...
// ─────────────────────────────────────────────────────────────
//  KOMPONEN ListInput (Logika Perbaikan Tambah Baris)
// ─────────────────────────────────────────────────────────────
function ListInput({
  ...
  return (
    <div className="space-y-1.5">
        ...
        <div key={idx} className="flex items-center gap-2 group">
          ...
          {items.length > 1 && (
            <button onClick={() => removeItem(idx)}
              className="text-red-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all text-lg leading-none flex-shrink-0 w-5 text-center">
              ×
            </button>
          )}
        </div>
      ...
      <button onClick={addItem}
        className="text-xs text-emerald-600 hover:text-emerald-700 hover:underline flex items-center gap-1 pl-6 mt-1">
        + Tambah item
      </button>
    </div>
  )
          ...
          return (
            <div key={santri.id}>
              <button onClick={() => toggle(santri.id)}
                ...
                <span className="w-7 h-7 rounded-full bg-emerald-50 text-emerald-600 text-xs font-semibold flex items-center justify-center flex-shrink-0">
                  {santri.no_urut}
                </span>
              ...
              {isOpen && (
                <div className="px-4 pb-4 bg-gray-50/50">
                  {santri.prestasi.length === 0 && <p className="text-xs text-gray-400 py-3 pl-10">Belum ada data yang diisi.</p>}
                  {santri.prestasi.map((p, pi) => (
                    <div key={p.id} className="ml-10 mt-3 bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
                      <p className="text-xs text-gray-400 mb-3">
                        Entri {pi + 1} · {new Date(p.updated_at).toLocaleDateString('id-ID', { day:'numeric', month:'short', year:'numeric' })}
                      </p>
  ...
  if (saved && selectedSantri) return (
    <div className="bg-white border border-gray-200 rounded-2xl text-center py-10 px-6 shadow-sm">
      ...
      <div className="flex flex-col gap-3 max-w-xs mx-auto">
        ...
        <button onClick={() => { setSelected(null); setSaved(false) }} className="w-full bg-gray-100 text-gray-600 py-2.5 rounded-xl font-semibold hover:bg-gray-200 transition-colors">
          Kembali ke Daftar
        </button>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-6">
        <div>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">1. Kegiatan Sekolah</p>
        ...
        <div className="pt-6 border-t border-gray-100">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">2. Kegiatan Pondok</p>
          <div className="flex flex-wrap gap-2 mb-4">
            {KEGIATAN_PON.map(t => (
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">3. Prestasi</p>
        <div className="space-y-4">
          <div>
            <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Di Sekolah</label>
            <ListInput value={form.prestasi_sekolah} onChange={v => setForm(f => ({...f, prestasi_sekolah: v}))} placeholder="Contoh: Juara 1 Lomba Pidato" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Di Pondok</label>
            <ListInput value={form.prestasi_pondok} onChange={v => setForm(f => ({...f, prestasi_pondok: v}))} placeholder="Contoh: Khatam Imriti 100 Bait" />
          </div>
        </div>
      </div>

      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">4. Progres Pribadi</p>
        <div className="flex flex-wrap gap-2 mb-4">
          {PROGRES_TAGS.map(t => (
            <button key={t} onClick={() => setForm(f => ({ ...f, progres_pribadi: f.progres_pribadi.trim() ? f.progres_pribadi + '\n' + t : t }))}
              className="px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-200 text-xs text-gray-600 hover:bg-emerald-50 hover:border-emerald-200 transition-colors">
              + {t}
            </button>
          ))}
        </div>
        <ListInput value={form.progres_pribadi} onChange={v => setForm(f => ({...f, progres_pribadi: v}))} placeholder="Ceritakan perkembangan santri..." />
      </div>
          ...
          <div className="space-y-6">
            <div className="flex bg-gray-200/50 p-1 rounded-xl">
              <button onClick={() => setActiveTab('input')}
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'input' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                ✏️ INPUT DATA
              </button>
              <button onClick={() => setActiveTab('rekap')}
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'rekap' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                📋 REKAP KELAS
              </button>
            </div>
